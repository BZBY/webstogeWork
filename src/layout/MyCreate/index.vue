...
onMounted(() => {
  ...
  if (editFlag === "1") {
    ...
    if (tmpWords) {
      ...
      if (data.author === JSON.parse(localStorage.getItem("NowUser")).nickname) {
    ...
    if (
      ...
      if (editFlag === "1") {
        ...
        if (index !== -1) {
          ...
          existingArticle.tag = article.tag;
          existingArticle.content = article.content;
          existingArticle.desc = "desc : " + article.content.slice(0, 15) + (article.content.length > 15 ? "..." : "");
        }
...
const save = () => {
  if (!User.isLogin) {
    ...
    if (
      ...
      // 更新文章数据
      localStorage.setItem('articles', JSON.stringify(articles));
      localStorage.setItem("tmpWords", JSON.stringify(existingArticle));
